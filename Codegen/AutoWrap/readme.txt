...
The remainder of the file will provide some documentation for the 
"Attributes.xml" file.
...
"meta.xml" file. Each element is identified by
  ...
  * its position in the XML tree (e.g. "<meta>" -> "<namespace>" -> "class")
  * the element type (e.g. "<class>", "<typedef>", "<variable>", ...)
  * the element's name (i.e. the name attribute)
  
Therefore a class in the "Ogre" namespace (note that its the C++ namespace, not
the C++/CLI namespace - if they're different) named "ScriptToken" would
